			...
			<c:forEach begin="1" end="${totalPage }" var="page">
				<c:choose>
					<c:when test="${itemVO.category03Code ne 0 }">
						<a href="itemMenuCategory.do?pageIndex=${page }&category03Code=${itemVO.category03Code }">${page }</a>
					</c:when>
					<c:when test="${itemVO.categoryCode ne 0 }">
						<a href="itemMenuCategory.do?pageIndex=${page }&categoryCode=${itemVO.categoryCode }">${page }</a>
					</c:when>
					<c:otherwise>
						<a href="itemMenuCategory.do?pageIndex=${page }">${page }</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
		<div style="height: 20px;"></div>
